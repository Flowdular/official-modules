...
export class TargetIdempotencyConflictError extends Error {
	readonly code = 'PARTY_IDEMPOTENCY_CONFLICT';

	constructor() {
		super('The idempotency key is bound to another party operation or input.');
		this.name = 'TargetIdempotencyConflictError';
	}
}

export class TargetIdempotencyCorruptionError extends Error {
	readonly code = 'PARTY_IDEMPOTENCY_LEDGER_CORRUPT';

	constructor() {
		super('The party idempotency ledger contains an invalid result.');
